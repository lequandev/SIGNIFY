...
import { Ban, Building2, CheckCircle2, Copy, Cpu, GraduationCap, KeyRound, ListVideo, Loader2, Mail, Pencil, Save, Shield, Trash2, UserPlus, Users, X } from 'lucide-react';
...
const SchoolPage: React.FC = () => {
  ...
  const workspaceView = requestedTab === 'history' ? 'HISTORY' : requestedTab === 'ai-usage' ? 'AI_USAGE' : 'MEMBERS';
  const setWorkspaceView = (view: 'MEMBERS' | 'HISTORY' | 'AI_USAGE') => {
    const next = new URLSearchParams(searchParams);
    if (view === 'HISTORY') next.set('tab', 'history');
    else if (view === 'AI_USAGE') next.set('tab', 'ai-usage');
      ...
      <main className="pb-20">
        ...
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-10">
          {loading ? <div className="flex min-h-[360px] items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div> : !school ? <div className="rounded-2xl border border-dashed border-outline-variant bg-surface-container-lowest px-6 py-16 text-center"><Building2 className="mx-auto h-10 w-10 text-outline" /><h2 className="mt-4 text-lg font-extrabold">Chưa có không gian trường học</h2><p className="mt-2 text-sm text-on-surface-variant">Tài khoản này chưa được liên kết với một trường học đang hoạt động.</p></div> : <>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Stat icon={Building2} label="Trạng thái" value={statusLabel(school.status)} /><Stat icon={Users} label="Thành viên hoạt động" value={activeMembers} /><Stat icon={GraduationCap} label="Giáo viên" value={school.teacherCount} /><Stat icon={Shield} label="Học sinh" value={school.studentCount} /></div>
            {school.canManageMembers && <div className="mt-6 inline-flex max-w-full overflow-x-auto rounded-xl border border-outline-variant/55 bg-surface-container p-1"><button type="button" onClick={() => setWorkspaceView('MEMBERS')} className={`inline-flex h-9 shrink-0 items-center gap-2 rounded-lg px-4 text-xs font-extrabold transition ${workspaceView === 'MEMBERS' ? 'bg-surface-container-lowest text-primary shadow-sm' : 'text-on-surface-variant hover:text-on-surface'}`}><Users className="h-4 w-4" />Thành viên</button><button type="button" onClick={() => setWorkspaceView('HISTORY')} className={`inline-flex h-9 shrink-0 items-center gap-2 rounded-lg px-4 text-xs font-extrabold transition ${workspaceView === 'HISTORY' ? 'bg-surface-container-lowest text-primary shadow-sm' : 'text-on-surface-variant hover:text-on-surface'}`}><ListVideo className="h-4 w-4" />Lịch sử xem</button><button type="button" onClick={() => setWorkspaceView('AI_USAGE')} className={`inline-flex h-9 shrink-0 items-center gap-2 rounded-lg px-4 text-xs font-extrabold transition ${workspaceView === 'AI_USAGE' ? 'bg-surface-container-lowest text-primary shadow-sm' : 'text-on-surface-variant hover:text-on-surface'}`}><Cpu className="h-4 w-4" />AI Usage</button></div>}
